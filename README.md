...
<!-- default badges end -->
# DevExtreme FilterBuilder - Enable Search in Field Dropdowns

This example implements search within field dropdowns of the DevExtreme [FilterBuilder](https://js.devexpress.com/Documentation/Guide/UI_Components/FilterBuilder/Overview/).

![DevExtreme FilterBuilder - Enable Search in Field Dropdowns](images/search-enabled-for-dropdown.gif)

## Implementation Details 

FilterBuilder uses the DevExtreme [TreeView](https://js.devexpress.com/Documentation/Guide/UI_Components/TreeView/Overview/) component to display available field values and operations.

To determine if a TreeView instance is generated by the FilterBuilder, check if the component has the `cssClass` option (using [String.includes()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/includes)). This option is not available in standalone TreeView components. FilterBuilder adds class names to `cssClass` values as follows:
- `dx-filterbuilder-fields`: Added for the Field Values dropdown.
- `dx-filterbuilder-operations`: Added for the Operations dropdown.

Note that in the DOM, these classes are added to the wrapper element of the TreeView's parent Popup.
